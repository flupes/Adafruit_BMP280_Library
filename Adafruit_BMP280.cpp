 ...
 *         optional wire
 */
Adafruit_BMP280::Adafruit_BMP280(TwoWire *theWire) { _wire = theWire; }
 ...
 */
void Adafruit_BMP280::setSampling(sensor_mode mode, sensor_sampling tempSampling,
...
int16_t Adafruit_BMP280::readS16_LE(byte reg) { return (int16_t)read16_LE(reg); }
 ...
 * @return The temperature in hundreds of degrees celcius.
 */
int16_t Adafruit_BMP280::readTemperatureCentiDegrees() {
  int32_t var1, var2;
  ...
  var1 =
      ((((adc_T >> 3) - ((int32_t)_bmp280_calib.dig_T1 << 1))) * ((int32_t)_bmp280_calib.dig_T2)) >>
      11;
...
float Adafruit_BMP280::readTemperature() {
  ...
  return static_cast<float>(readTemperatureCentiDegrees()) / 100.0f;
}

uint32_t Adafruit_BMP280::readAndProcessPressure() {
  int64_t var1, var2, p;
 ...
 * @return Barometric pressure in DeciPa.
 */
uint32_t Adafruit_BMP280::readPressureDeciPa() {
  // Must be done first to get the t_fine variable set up
  readTemperatureCentiDegrees();

  return readAndProcessPressure();
}
...
float Adafruit_BMP280::readPressure() {
  ...
  return static_cast<float>(readPressureDeciPa()) / 10.0f;
}

bool Adafruit_BMP280::readPressureAndTemperature(uint32_t &pressure, int16_t &temperature) {
  if (!_sensorID) {
    return false;
  }
  temperature = readTemperatureCentiDegrees();
  pressure = readAndProcessPressure();
  return true;
}
 ...
 */
void Adafruit_BMP280::reset(void) { write8(BMP280_REGISTER_SOFTRESET, MODE_SOFT_RESET_CODE); }
 ...
 */
uint8_t Adafruit_BMP280::getStatus(void) { return read8(BMP280_REGISTER_STATUS); }
